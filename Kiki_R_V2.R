...
##iii.select the data type:class()、str()
my_numeric <- 60
my_character <- "Taiwan"
my_logical <- FALSE
class(my_numeric)
...
##useful function(levels()、min()、max()、median()、length()、prod()、sum()、mean())
##levels,min,max
...
##ii.Recognize "cbind、rbind、colnames、rownames、dimnames、lettlers、LETTERS、dim、sample"
#cbind,rbind
cbind(1:3,1:3)
...
##iii.Use cblind、rbind and vector to create matrix row and column 
m<-matrix(1:6,byrow=TRUE,nrow=2)
m
...
##ii.Extend data frame(create row)、use cblind、rblind create row and column
name<-c("Anne","Pete","Frank","Julia","Cath")
age<-c(28,30,21,39,35)
child<-c(FALSE,TRUE,TRUE,FALSE,TRUE)
df<-data.frame(name,age,child)
...
##viii.arrange()??row?C????(from Internet)
View(newmurders)
